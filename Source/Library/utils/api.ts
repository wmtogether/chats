...
/**
 * Fetches a single chat by its UUID.
 * Returns a promise that resolves to the API response.
 */
export async function getChat(chatUuid: string): Promise<{
  success: boolean;
  message?: string;
  error?: string;
  data?: ChatType;
 ...
 * Returns a promise that resolves to the API response with customer data.
 */
export async function fetchCustomers(): Promise<{
  success: boolean;
  message?: string;
  error?: string;
  data?: import('../types').CustomerType[];
}> {
  try {
    const token = localStorage.getItem('authToken');
    if (!token) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new Error('You don\'t have permission to view customers.');
      } else {
        throw new Error(result.error || `HTTP error! status: ${response.status}`);
      }
    }
...
/**
 * Creates a new customer with auto-generated customer ID.
 * Returns a promise that resolves to the API response with created customer data.
 */
export async function createCustomer(customerName: string): Promise<{
  success: boolean;
  message?: string;
  error?: string;
  data?: import('../types').CustomerType;
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/customers`, {
      ...
      body: JSON.stringify({
        ...
        // cusId will be auto-generated by the backend
      }),
    });
    ...
    if (!response.ok) {
      // Handle specific error cases
      if (response.status === 400) {
        throw new Error(result.error || 'Invalid customer data provided.');
      } else if (response.status === 401) {
        throw new Error('Authentication required. Please log in again.');
      } else if (response.status === 403) {
 ...
 * Returns a promise that resolves to the API response.
 */
export async function updateChatStatus(chatUuid: string, status: string): Promise<{
  success: boolean;
  message?: string;
  error?: string;
  data?: ChatType;
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/chats/${chatUuid}/status`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      // Handle specific error cases
      if (response.status === 400) {
        throw new Error(result.error || 'Invalid status value provided.');
      } else if (response.status === 401) {
        throw new Error('Authentication required. Please log in again.');
      } else if (response.status === 403) {
        throw new Error(result.error || 'You don\'t have permission to update this chat status. Only the creator or administrators can update chats.');
      } else if (response.status === 404) {
        throw new Error('Chat not found.');
      } else {
        throw new Error(result.error || `HTTP error! status: ${response.status}`);
